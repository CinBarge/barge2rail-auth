...
from sso.auth_views import google_auth_callback, login_google
...
def test_oauth_state_generation():
    ...
    request = factory.get(
        "/api/auth/login/google/?next=/dashboard/", SERVER_NAME="127.0.0.1"
    )
...
def test_oauth_state_verification():
    ...
    factory = RequestFactory()
    request = factory.get(
        "/api/auth/google/callback/?code=test_code", SERVER_NAME="127.0.0.1"
    )
    ...
    request = factory.get(
        "/api/auth/google/callback/?code=test_code&state=wrong_state",
