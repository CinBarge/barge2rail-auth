...
class AdminOAuthLoginViewTests(TestCase):
    ...
    @override_settings(GOOGLE_CLIENT_ID="", GOOGLE_CLIENT_SECRET="test-secret")
    @patch("sso.admin_oauth_views.GOOGLE_AUTH_AVAILABLE", True)
...
class AdminOAuthCallbackViewTests(TestCase):
    ...
    def test_invalid_state_redirects_to_login(self):
        ...
        response = self.client.get(
            self.url + "?code=test_code&state=invalid_state:654321", follow=True
        )
    ...
    @override_settings(
        ADMIN_WHITELIST="admin@example.com", SUPERUSER_WHITELIST="admin@example.com"
    )
    ...
    def test_successful_admin_oauth_flow(
        ...
        response = self.client.get(self.url + f"?code=test_code&state={state}")
    ...
    @override_settings(ADMIN_WHITELIST="", SUPERUSER_WHITELIST="")
    @patch("sso.admin_oauth_views.authenticate")
    @patch("sso.admin_oauth_views.validate_google_id_token")
    @patch("sso.admin_oauth_views.exchange_google_code_for_tokens")
    def test_non_admin_user_denied_access(
        ...
        # Make callback request
        response = self.client.get(
            self.url + f"?code=test_code&state={state}", follow=True
        )

        # Should redirect to admin login
        self.assertRedirects(response, "/admin/login/")

    @override_settings(ADMIN_WHITELIST="user@example.com", SUPERUSER_WHITELIST="")
    @patch("sso.admin_oauth_views.authenticate")
    @patch("sso.admin_oauth_views.validate_google_id_token")
    @patch("sso.admin_oauth_views.exchange_google_code_for_tokens")
