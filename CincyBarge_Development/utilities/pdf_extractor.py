...
class BOLPDFExtractor:
    ...
    def _extract_shipper_address(self, text: str) -> Optional[str]:
        ...
        if shipper_match:
            ...
            address = re.split(
                r"(?:Consignee|To|Carrier|Notify)", address, flags=re.IGNORECASE
            )[0]
    ...
    def _extract_consignee_address(self, text: str) -> Optional[str]:
        ...
        if consignee_match:
            ...
            # Stop at next section header
            address = re.split(
                r"(?:Notify|Carrier|Container|Vessel)", address, flags=re.IGNORECASE
            )[0]
            return address.strip()
        return None
    ...
    def _extract_line_items(
        ...
        for table in tables:
            ...
            desc_col = self._find_column_index(
                headers, ["description", "product", "item", "commodity"]
            )
            qty_col = self._find_column_index(
                headers, ["quantity", "qty", "pieces", "pcs"]
            )
            weight_col = self._find_column_index(
                headers, ["weight", "wt", "gross weight", "net weight"]
            )
